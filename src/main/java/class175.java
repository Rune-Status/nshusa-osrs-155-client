public class class175 {
	...
	static final void method3469(int var0, int var1, int var2, int var3) {
		++Client.field1016;
		if (AbstractByteBuffer.localPlayer.x >> 7 == Client.destinationX
				&& AbstractByteBuffer.localPlayer.y >> 7 == Client.destinationY) {
		...
		if (!Client.field1160) {
			...
			class15.method97(
					Occluder.field2140, CacheFile.getTileHeight(AbstractByteBuffer.localPlayer.x,
							AbstractByteBuffer.localPlayer.y, class5.plane) - Client.field1002,
					class40.field569, var4, var14, var4 * 3 + 600);
		}
		...
		if (!Client.field1160) {
			...
			if (class23.preferences.hideRoofs) {
				...
				if (var6 - ScriptState.cameraZ < 800
						&& (class61.tileSettings[class5.plane][class211.cameraX >> 7][ChatLineBuffer.cameraY >> 7]
								& 4) != 0) {
					var14 = class5.plane;
				} else {
					var14 = 3;
		...
		for (var10 = 0; var10 < 5; ++var10) {
			if (Client.field1161[var10]) {
				var11 = (int) (Math.random() * (double) (Client.field1162[var10] * 2 + 1)
						- (double) Client.field1162[var10]
						+ Math.sin((double) Client.field1164[var10] / 100.0D * (double) Client.field1165[var10])
								* (double) Client.field1056[var10]);
				if (var10 == 0) {
		...
		class8.region.draw(class211.cameraX, ScriptState.cameraZ, ChatLineBuffer.cameraY, class9.cameraPitch,
