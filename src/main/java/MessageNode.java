	...
	static final void method1153(Actor var0, int var1, int var2, int var3, int var4, int var5) {
		if (var0 != null && var0.hasConfig()) {
			...
			if (!var0.combatInfoList.method3667()) {
				...
				for (CombatInfoListHolder var9 = (CombatInfoListHolder) var0.combatInfoList
						.method3664(); var9 != null; var9 = (CombatInfoListHolder) var0.combatInfoList.method3665()) {
			...
			if (var1 < var74) {
				...
				if (var85.skullIcon != -1 || var85.overheadIcon != -1) {
					...
					if (Client.screenY > -1) {
						if (var85.skullIcon != -1) {
							FaceNormal.mapfunctions[var85.skullIcon].method5172(var2 + Client.screenY - 12,
									var3 + Client.screenX - var8);
							var8 += 25;
						}

						if (var85.overheadIcon != -1) {
							class35.hitmarks[var85.overheadIcon].method5172(var2 + Client.screenY - 12,
									var3 + Client.screenX - var8);
							var8 += 25;
						}
					}
				...
				if (var86.field3584 >= 0 && var86.field3584 < class35.hitmarks.length) {
					class72.characterToScreen(var0, var0.field1257 + 15);
					if (Client.screenY > -1) {
						class35.hitmarks[var86.field3584].method5172(var2 + Client.screenY - 12,
								var3 + Client.screenX - 30);
					}
				}

				if (Client.field938 == 1 && Client.npcIndices[var1 - var74] == Client.field939
			...
			if (var0.overhead != null && (var1 >= var74 || !var0.inSequence
					&& (Client.field1129 == 4 || !var0.field1221 && (Client.field1129 == 0 || Client.field1129 == 3
							|| Client.field1129 == 1 && class44.isFriended(((Player) var0).name, false))))) {
						...
						if (var0.itemIds[var13] > 0) {
							...
							if (Client.itemSelectionState == 1 && class94.method1858(class244.getWidgetConfig(var0))) {
								if (var0.id != class25.field371 || var13 != class39.selectedItemIndex) {
									class10.addMenuEntry("Use", Client.field1067 + " " + "->" + " "
											+ class60.getColTags(16748608) + var8.name, 31, var8.id, var13, var0.id);
								}
								...
								if ((WorldMapType1.field451 & 16) == 16) {
									class10.addMenuEntry(
											Client.field1071, Client.field948 + " " + "->" + " "
													+ class60.getColTags(16748608) + var8.name,
											32, var8.id, var13, var0.id);
								}
							} else {
								...
								if (class33.method338(class244.getWidgetConfig(var0))) {
									class10.addMenuEntry("Use", class60.getColTags(16748608) + var8.name, 38, var8.id,
											var13, var0.id);
								}
								...
								if (var9 != null) {
									for (var11 = 4; var11 >= 0; --var11) {
										if (var9[var11] != null) {
											...
											class10.addMenuEntry(var9[var11], class60.getColTags(16748608) + var8.name,
													var12, var8.id, var13, var0.id);
										}
									}
								}

								class10.addMenuEntry("Examine", class60.getColTags(16748608) + var8.name, 1005, var8.id,
										var13, var0.id);
							}
						}
					}
		...
		if (var0.hasScript) {
			if (Client.spellSelected) {
				if (class61.method1122(class244.getWidgetConfig(var0)) && (WorldMapType1.field451 & 32) == 32) {
					class10.addMenuEntry(Client.field1071, Client.field948 + " " + "->" + " " + var0.name, 58, 0,
							var0.index, var0.id);
				}
			} else {
				for (var13 = 9; var13 >= 5; --var13) {
					...
					if (!class110.method2171(class244.getWidgetConfig(var0), var13) && var0.field2758 == null) {
						var14 = null;
					} else if (var0.actions != null && var0.actions.length > var13 && var0.actions[var13] != null
							&& var0.actions[var13].trim().length() != 0) {
				...
				for (var4 = 4; var4 >= 0; --var4) {
					...
					if (!class110.method2171(class244.getWidgetConfig(var0), var4) && var0.field2758 == null) {
						var15 = null;
					} else if (var0.actions != null && var0.actions.length > var4 && var0.actions[var4] != null
							&& var0.actions[var4].trim().length() != 0) {
